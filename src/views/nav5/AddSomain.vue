        ...
        <Form ref="formValidate" id="fromtable" label-position="left" :model="formValidate" :rules="ruleValidate" :label-width="80" >
          ...
          <div style="margin-top:10px;padding-left: 10px;">
            <FormItem label="销售单编号" prop="soId"  >
              <Input v-model="formValidate.soId" placeholder="Enter your poId" class="inps" ></Input>
            </FormItem>
            ...
            <FormItem label="采购产品总价" prop="soTotal">
              <Input v-model="formValidate.soTotal" placeholder="Enter your soTotal" class="inps" ></Input>
            </FormItem>
            </div>
        </Form>
        <!-- 添加明细 -->
        <Table ref="currentRowTable" :columns="columns" :data="formValidate.soitems" border style="width: 1000px; margin-top: 10px">
          <template slot-scope="{ row, index }" slot="productCode" >
            <Input v-model="formValidate.soitems[index].productCode" placeholder="Enter something..." style="width:150px;position: relative;top:15px"></Input>
            <Icon type="ios-create"  @click="showModal(index)" style="font-size:30px;position: relative;right:-150px;top:-17px"/></template>
          <template slot-scope="{ row, index }" slot="price">
            <Input v-model="formValidate.soitems[index].unitPrice" placeholder="Enter something..." ></Input></template>
          <template slot-scope="{ row, index }" slot="num">
            <Input v-model="formValidate.soitems[index].num" placeholder="Enter something..."></Input></template>
          <template slot-scope="{ row, index }" slot="unitName">
            <Input v-model="formValidate.soitems[index].unitName" placeholder="Enter something..." ></Input></template>
          <template slot-scope="{ row, index }" slot="name">
            <Input v-model="formValidate.soitems[index].name" placeholder="Enter something..." ></Input></template>
          <template slot-scope="{ row, index }" slot="itemPrice">
            <Input v-model="formValidate.soitems[index].itemPrice" placeholder="Enter something..." ></Input></template>
          <template slot-scope="{ row, index }" slot="action">
            <Button type="error" size="small" @click="remove(index)">删除</Button>
          </template>
        </Table>
        <Modal v-model="modal1" title="销售单明细"  :width="630" :height="200" >
...
export default {
  data() {
    return {
      ...
      ruleValidate: {
        ...
        soitems: [ { required: true, message: "The name cannot be empty", trigger: "blur", }, ],
      },
      currentTime: '',
      timer: "",
      currentRow:'',
      ...
      columns: [
        ...
        {title: "产品单价", slot: "price",width: 100},
        {title: "产品数量",slot: "num",width: 100},
        {title: "名称",slot: "name"},
  ...
  methods:{
    ...
    add(){
      this.formValidate.soitems.push({
        productCode:"",
        rice:'',
        num:'',
        unitName:'',
    ...
    SubmitChoise(row){
      this.formValidate.soitems[this.currentRow].productCode=row.productCode;
      this.formValidate.soitems[this.currentRow].unitPrice=row.price
      this.formValidate.soitems[this.currentRow].num=row.num
      this.formValidate.soitems[this.currentRow].name=row.name
      this.formValidate.soitems[this.currentRow].unitName=row.unitName
      this.formValidate.soitems[this.currentRow].itemPrice=row.price*row.num
      this.modal1=false
    },
    //删除
    remove() {
      ...
      content: '<p>是否要删除本条数据？</p>',
        onOk: () => {
          this.formValidate.soitems.splice(this.currentRow, 1),this.$Message.success('删除成功');},
        onCancel: () => {this.$Message.info('取消删除');}});
    },
    //保存
    handleSubmit(){
      this.formValidate.createTime=this.currentTime
      this.formValidate.productTotal=this.formValidate.soitems[this.currentRow].itemPrice
      this.formValidate.soitems[this.currentRow].itemPrice=
              parseInt(this.formValidate.soitems[this.currentRow].num)*
              parseInt(this.formValidate.soitems[this.currentRow].unitPrice)  
      this.formValidate.soTotal=
