    ...
    <Layout>
      <Header id="header" style="border-bottom:1px solid #ccc" >
        <Breadcrumb separator=">">
          ...
          <BreadcrumbItem to="">收款登记</BreadcrumbItem>
        </Breadcrumb>
      </Header>
      <Content style="margin: 30px 50px">
          <Tabs type="card" :animated="false" @on-click="tabsclick">
              ...
              <TabPane label="收预付款到发货" name="3"></TabPane>
          </Tabs>
          <Table ref="currentRowTable" :columns="columns" :data="pageData.list" border style="width: 1000px; margin-top: 10px">
              <template slot-scope="{ row, index }" slot="action">
                <Button type="success" size="small" @click="receipt(row)" 
                  v-show="(row.payType=='1'&&row.status=='2')||(row.payType=='2'&&row.status=='1')">
                    收款</Button>
                <Button type="info" size="small" @click="receipt2(row)" 
                  v-show="row.payType=='3'&&row.status=='1'">收预款</Button>
                <Button type="warning" size="small" @click="receipt(row)" 
                  v-show="row.payType=='3'&&row.status=='2'">收尾款</Button>
                </template>
          </Table>
...
export default {
  data() {
    return {
      ...
      columns: [
        ...
        {title: "采购单编号", key:"soId" ,width: 150,align: "center"},
        {title: "创建时间",key:"createTime",width: 120},
        {title: "供应商名称",key:"customerName", width: 120, align: "center" },
        {title: "创建用户",key:"account",width:80},
  ...
  methods:{
    tabsclick(name){
      axios.post("/cgi/main/sell/somain/show?page=1"+"&type=3"+"&payType="+name)
      .then((resp) => {
    ...
    //分页
    changepage(page) {
      axios.post("/cgi/main/sell/somain/show?page="+page+"&type=3"+"&payType="+name)
      .then((resp) => {
        this.pageData = resp.data;
      })
    },
    receipt(row){
      this.$Modal.confirm({
        ...
        content: '<p>是否要收款？</p>',
        onOk: () => {
          axios.post("/cgi/main/finance/receipt?soId="+row.soId+"&type=1").then((resp) => {
    ...
    receipt2(row){
      this.$Modal.confirm({
        ...
        content: '<p>是否要付款？</p>',
        onOk: () => {
          axios.post("/cgi/main/finance/receipt?soId="+row.soId+"&type=2")
          .then((resp) => {
            this.$Message.success('success');
            })},
        onCancel: () => {this.$Message.info('取消操作');}});
