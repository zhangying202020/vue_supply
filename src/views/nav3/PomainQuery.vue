    ...
    <Layout>
      <Header id="header" style="border-bottom:1px solid #ccc" >
        <Breadcrumb separator=">">
          ...
          <BreadcrumbItem to="">采购单查询</BreadcrumbItem>
        </Breadcrumb>
      </Header>
      <Content style="margin: 30px 50px">
        <Form ref="formInline" :model="formInline" inline>
            <FormItem label="" prop="poId" >
                <span>采购单编号</span>
                <Input type="text" v-model="formInline.poId" style="width:150px" ></Input>
            </FormItem>
            <FormItem prop="date">
...
export default {
  ...
  methods:{
    ...
    search(name) {
        this.$refs[name].validate((valid) => {
            if (valid) {
                ...
                axios.post("/cgi/main/purchase/pomain/query",qs.stringify(this.formInline)).then((resp) => {
    ...
    //分页
    changepage(page) {
      axios.post("/cgi/main/purchase/pomain/query?page="+page).then((resp) => {
        this.pageData = resp.data;
      })
    },
  },
