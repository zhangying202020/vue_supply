  ...
  <div class="layout">
    <Layout>
      ...
      <Content style="margin: 30px 50px">
        <Button type="success" to="/homepage/pomain/addpomain" style="width: 100px" >
        ...
        <Form ref="formValidate" id="fromtable" label-position="left" :model="formArr" :rules="ruleArr" :label-width="80" >
          <div style="margin-top:10px;padding-left: 10px;">
            <FormItem label="采购单编号" prop="poId"  >
              <Input v-model="formArr.poId" placeholder="Enter your poId" class="inps" ></Input>
            </FormItem>
            ...
            <FormItem label="供应商编号" prop="venderCode" class="FormItemleft" >
              <Input v-model="formArr.venderCode" placeholder="Enter your venderCode" class="inps" ></Input>
            </FormItem>
            </div>
            ...
            <FormItem label="采购产品总价" prop="poTotal">
              <Input v-model="formArr.poTotal" placeholder="Enter your poTotal" class="inps" ></Input>
            </FormItem>
            </div>
            <div style="padding-left: 10px;">
        ...
        <!-- 添加明细 -->
        <Table ref="currentRowTable" :columns="columns" :data="formArr.poitems" border style="width: 1000px; margin-top: 10px">
          <template slot-scope="{ row, index }" slot="productCode" >
            <Input v-model="formArr.poitems[index].productCode" placeholder="Enter something..." ></Input>
            </template>
          <template slot-scope="{ row, index }" slot="price">
            <Input v-model="formArr.poitems[index].unitPrice" placeholder="Enter something..." ></Input></template>
          <template slot-scope="{ row, index }" slot="num">
            <Input v-model="formArr.poitems[index].num" placeholder="Enter something..."></Input></template>
          <template slot-scope="{ row, index }" slot="unitName">
            <Input v-model="formArr.poitems[index].unitName" placeholder="Enter something..." ></Input></template>
          <template slot-scope="{ row, index }" slot="itemPrice">
            <Input v-model="formArr.poitems[index].itemPrice" placeholder="Enter something..." ></Input></template>
          <template slot-scope="{ row, index }" slot="action">
...
export default {
  data() {
    return {
      ...
      columns1: [
        ...
        {title: "采购单编号", key: "poId",width: 150,classHeader: 'HeaderStyle',},
        {title: "供应商编号", key: "venderCode",width: 80,classHeader: 'HeaderStyle',},
        {title: "创建用户",key: "account",width: 70,classHeader: 'HeaderStyle',},
        {title: "创建时间",key: "createTime",width: 110,classHeader: 'HeaderStyle',},
        {title: " 附加费用",key: "tipFee",width: 55,classHeader: 'HeaderStyle',},
        {title: "产品总价", key: "productTotal",width: 70,classHeader: 'HeaderStyle',},
        {title: "采购总价", key: "poTotal",width: 80,classHeader: 'HeaderStyle',},
        {title: "付款方式", key: "payType",width: 50,classHeader: 'HeaderStyle',},
        {title: "采购单状态", key: "status",width: 50,classHeader: 'HeaderStyle',},
        {title: " 备注", key: "remark",width: 58,classHeader: 'HeaderStyle',},
  ...
  mounted() {
    axios.get("/cgi/main/purchase/pomain/show?type="+1+"&page="+1).then((resp) => {
        this.pageData = resp.data;});
  },
    methods:{
    //分页
    changepage(page) {
      axios.post("/cgi/main/purchase/pomain/show?type="+1+"&page="+page).then((resp) => {
        this.pageData = resp.data;
      })
    },
    //明细单删除
    remove(index) {
      ...
      content: '<p>是否要删除本条数据？</p>',
        onOk: () => {this.formArr.poitems.splice(index, 1),this.$Message.success('删除成功');},
    ...
    remove(row) {
      this.$Modal.confirm({
        ...
        content: '<p>是否要删除本条数据？</p>',
        onOk: () => {
          axios.post("/cgi/main/purchase/pomain/delete?poId="+row.poId).then((resp) => {
    ...
    update(row){
      this.modal = true
      axios.post("/cgi/main/purchase/pomain/queryItem?poId="+row.poId).then((resp) => {
        this.formArr=row
        this.formArr.poitems=resp.data
      });
    },
    //保存
    handleSubmit(){
      this.formArr.poTotal=
